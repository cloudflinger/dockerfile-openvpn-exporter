...
RUN git clone https://github.com/kumina/openvpn_exporter.git /go/src/app
...
COPY --from=builder /go/src/app/examples/server2.status /etc/openvpn_exporter/server.status
